...
describe('BioPortrait', () => {
  it('renders <img> via RN-web when portraitUrl present', () => {
    const { container } = render(
      <BioPortrait fullName="Nancy Pelosi" portraitUrl="https://example.org/np.jpg" size={72} />,
    )
  ...
  it('initials text uses semantic.portrait.initials (light = white, dark = cream)', () => {
    const light = render(<BioPortrait fullName="Nancy Pelosi" portraitUrl={null} size={72} />, {
      wrapper: lightWrapper,
    })
    // Scope query to this render's container (multiple renders share document.body).
    const lightStyle =
      (within(light.container).getByText('NP') as HTMLElement).getAttribute('style') ?? ''
    expect(lightStyle).toMatch(/color:\s*(?:#ffffff|rgb\(255,\s*255,\s*255\))/i)

    const dark = render(<BioPortrait fullName="Nancy Pelosi" portraitUrl={null} size={72} />, {
      wrapper: darkWrapper,
    })
    const darkStyle =
      (within(dark.container).getByText('NP') as HTMLElement).getAttribute('style') ?? ''
