...
function mkState(
  ...
  return {
    ...
    last_name: '',
    bioguide_id: null,
    openstates_person_id: 'ocd-person/' + id,
    chamber,
    party: 'D',
    state: 'CA',
    district_id: 'did',
...
describe('StateOfficialsCardSection', () => {
  it('renders State heading + cards for each state official', () => {
    const officials = [mkState('state_house', 'Asm Test'), mkState('state_senate', 'Sen Test')]
  ...
  it('renders nothing when officials empty (DC user)', () => {
    const { container } = render(<StateOfficialsCardSection officials={[]} onSelect={vi.fn()} />)
  ...
  it('NE legislator labeled as State Senator (chamber=state_legislature)', () => {
    ...
    ne.state = 'NE'
    ne.title = 'Senator'
    const { getByText } = render(<StateOfficialsCardSection officials={[ne]} onSelect={vi.fn()} />)
...
describe('StateOfficialsCardSection — mode awareness', () => {
  it('renders under both light and dark wrappers without throwing', () => {
    ...
    const renderWith = (wrapper: typeof lightWrapper) =>
      render(<StateOfficialsCardSection officials={officials} onSelect={vi.fn()} />, { wrapper })
