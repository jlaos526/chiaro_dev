...
describe('BrandAlert', () => {
  ...
  it('exposes role=alert on the outer container', () => {
    const { container } = render(
      <BrandAlert severity="danger" title="Oops">
        body
      </BrandAlert>,
    )
  ...
  it('danger band uses burgundy #8a3a4d', () => {
    ...
    const allStyled = Array.from(
      container.querySelectorAll('[style*="background-color"]'),
    ) as HTMLElement[]
    const hasBand = allStyled.some((el) =>
      (el.getAttribute('style') ?? '').match(/background-color:\s*rgb\(138,\s*58,\s*77\)/),
    )
  ...
  it('warning band uses gold #c89a4e', () => {
    const { container } = render(
      <BrandAlert severity="warning" title="Heads up">
        body
      </BrandAlert>,
      { wrapper: lightWrapper },
    )
    ...
    const hasBand = allStyled.some((el) =>
      (el.getAttribute('style') ?? '').match(/background-color:\s*rgb\(200,\s*154,\s*78\)/),
    )
    expect(hasBand).toBe(true)
  })
  ...
  it('success band uses emerald #1a8f5a', () => {
    const { container } = render(
      <BrandAlert severity="success" title="Saved">
        body
      </BrandAlert>,
      { wrapper: lightWrapper },
    )
    ...
    const hasBand = allStyled.some((el) =>
      (el.getAttribute('style') ?? '').match(/background-color:\s*rgb\(26,\s*143,\s*90\)/),
    )
    expect(hasBand).toBe(true)
  })
  ...
  it('info band uses terracotta #b86340', () => {
    const { container } = render(
      <BrandAlert severity="info" title="FYI">
        body
      </BrandAlert>,
      { wrapper: lightWrapper },
    )
    ...
    const hasBand = allStyled.some((el) =>
      (el.getAttribute('style') ?? '').match(/background-color:\s*rgb\(184,\s*99,\s*64\)/),
    )
    expect(hasBand).toBe(true)
  })

  it('icon glyph differs per severity', () => {
    const dangerR = render(
      <BrandAlert severity="danger" title="x">
        y
      </BrandAlert>,
    )
    expect(dangerR.container.textContent).toContain('!')
    dangerR.unmount()
    const successR = render(
      <BrandAlert severity="success" title="x">
        y
      </BrandAlert>,
    )
    ...
    successR.unmount()
    const infoR = render(
      <BrandAlert severity="info" title="x">
        y
      </BrandAlert>,
    )
  ...
  describe('glyph color (slice 47 cleanup item 2)', () => {
    it('uses semantic.text.onAccent in light mode', () => {
      const { container } = render(<BrandAlert severity="info">Body</BrandAlert>, {
        wrapper: lightWrapper,
      })
