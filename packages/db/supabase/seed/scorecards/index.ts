...
export async function ingestScorecards(opts?: {
  congress?: string
  fixturesDir?: string
}): Promise<Record<string, { ratings: number; error?: string }>> {
    ...
    for (const a of ADAPTERS) {
      try {
        const fixturePath = opts?.fixturesDir
          ...
          : undefined
        const ratings = await a.fetchRatings(
          congress,
          fixturePath !== undefined ? { fixturePath } : {},
        )
        const orgRes = await client.query<{ id: string }>(
          'select id from public.scorecard_orgs where slug = $1',
          [a.slug],
        )
