...
import { caLeginfoOffices } from './ca-leginfo.ts'

const __dirname = dirname(fileURLToPath(import.meta.url))
const FIXTURE = join(__dirname, '..', '..', 'fixtures', 'state-community', 'offices-ca.json')

describe('ca-leginfo district-offices adapter', () => {
  it('happy path: fetcher injection returns fixture events', async () => {
    const fixture = JSON.parse(await readFile(FIXTURE, 'utf8'))
    const events = await caLeginfoOffices.fetchEvents({
      client: {} as never,
  ...
  it('production stub returns empty array', async () => {
    const events = await caLeginfoOffices.fetchEvents({ client: {} as never } as never)
  ...
  it('covered_states valid', () => {
    expect(caLeginfoOffices.covered_states.length).toBeGreaterThan(0)
    for (const s of caLeginfoOffices.covered_states) {
