...
describe('fetchColoradoRatings', () => {
  it('fetches both house + senate URLs and concatenates ratings', async () => {
    ...
    const ratings = await fetchColoradoRatings(
      client as never,
      {
        session: '2025',
        fetcher: async (url: string) => (url.includes('-house/') ? houseHtml : senateHtml),
      } as never,
    )
    // 4 House + 3 Senate = 7
