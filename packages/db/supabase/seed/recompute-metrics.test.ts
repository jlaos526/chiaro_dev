...
beforeEach(async () => {
  ...
  `)
  await client.query(
    "insert into public.bill_sponsors (bill_id, official_id, role, added_date) values ($1,$2,'sponsor','2026-01-10')",
    [b1.rows[0].id, officialId],
  )
  await client.query(
    "insert into public.bill_sponsors (bill_id, official_id, role, added_date) values ($1,$2,'cosponsor','2026-01-20')",
    [b2.rows[0].id, officialId],
  )

  // 1 district office, 1 town hall, 1 stock disclosure (on-time → days_late = 0)
  await client.query(
    "insert into public.district_offices (official_id, address, city, state, source_url) values ($1,'1 Main St','SF','CA','https://x/do')",
    [officialId],
  )
  await client.query(
    "insert into public.town_halls (official_id, event_date, city, state, format, source_url, source) values ($1,'2026-02-15','SF','CA','in_person','https://x/th','legacy')",
    [officialId],
  )
  await client.query(
    "insert into public.stock_transactions (official_id, transaction_date, filing_date, transaction_type, source_url, source) values ($1,'2026-01-10','2026-01-20','purchase','https://x/sk','legacy')",
    [officialId],
  )
})
...
afterEach(async () => {
  ...
  await client.query(
    "delete from public.votes where congress = '119' and roll_call in (9001, 9002)",
  )
...
describe('recomputeMetrics', () => {
  it('computes attendance, vote counts, bill counts, evidence counts, stock compliance', async () => {
    ...
    const m = await client.query('select * from public.official_metrics where official_id = $1', [
      officialId,
    ])
    expect(m.rows.length).toBe(1)
    const row = m.rows[0]

    expect(Number(row.attendance_pct)).toBe(50.0) // 1 voted / 2 total
    ...
    expect(Number(row.stock_act_compliance_pct)).toBe(100.0)
