...
export async function recomputeStateMetrics(
  ...
  try {
    ...
    for (const off of stateOfficials.rows) {
      ...
      const billPassageRate = sponsored === 0 ? null : (billsPassedCount / sponsored) * 100
      ...
      const totalRaised =
        financeRow.rows[0]?.total_raised == null ? null : Number(financeRow.rows[0]!.total_raised)
      const fiscalTotal = Number(billStats.rows[0]!.fiscal_total)
      const fiscalImpactPerDollarRaised =
        totalRaised == null || totalRaised === 0 ? null : fiscalTotal / totalRaised
