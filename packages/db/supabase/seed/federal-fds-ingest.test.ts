...
describe('ingestFederalFds happy path', () => {
  it('writes one row per holding + one row per other (bioguide path)', async () => {
    ...
    expect(holdingsInserts[0]?.sql).toMatch(/on conflict\s*\(source,\s*external_id\)\s+where\s+external_id\s+is\s+not\s+null/i)
    expect(otherInserts[0]?.sql).toMatch(/on conflict\s*\(source,\s*external_id\)\s+where\s+external_id\s+is\s+not\s+null/i)
  })
