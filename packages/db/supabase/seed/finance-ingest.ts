    ...
    for (const o of officials.rows) {
      ...
      try {
        const fixturePath = args.fixturesDir
          ...
          : undefined
        const snap: FinanceSnapshot = await fetcher(
          o.opensecrets_id,
          cycle,
          args.apiKey,
        ...
        `,
          [
            ...
            o.opensecrets_id,
            snap.total_raised,
            snap.total_disbursed,
            snap.small_donor_pct,
            snap.in_state_pct,
            snap.out_of_state_pct,
            snap.source_url,
          ],
        )
        ...
        await client.query(
          'delete from public.finance_industry_top where finance_summary_id = $1',
          [summaryId],
        )
        ...
        await client.query(
          'delete from public.finance_individual_donors where finance_summary_id = $1',
          [summaryId],
        )
        ...
        await client.query(
          'delete from public.finance_top_organizations where finance_summary_id = $1',
          [summaryId],
        )
...
if (isCliEntry(import.meta.url)) {
  ...
  if (!apiKey) {
    console.error('OPENSECRETS_API_KEY required')
    process.exit(1)
  }
