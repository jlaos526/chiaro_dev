...
describe('ingestLegislators', () => {
  it('populates opensecrets_id + fec_candidate_id + leadership history + district offices', async () => {
    const legYaml = await readFile(
      join(__dirname, 'fixtures/legislators-current-mini.yaml'),
      'utf8',
    )
    const offYaml = await readFile(
      join(__dirname, 'fixtures/legislators-district-offices-mini.yaml'),
      'utf8',
    )
