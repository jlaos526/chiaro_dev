...
beforeAll(async () => {
  ...
  svc = createClient<Database>(URL, SVC, {
    auth: { persistSession: false, storageKey: 'svc-bills-test' },
  })
  anon = createClient<Database>(URL, ANON, {
    auth: { persistSession: false, storageKey: 'anon-bills-test' },
  })
  ...
  await svc
    .from('vote_positions')
    .insert([{ vote_id: voteAId, official_id: officialId, position: 'yes' }])
})
  ...
  it('returns vote_positions with position = not_voting, scoped to the congress', async () => {
    ...
    const { data: v2 } = await svc
      ...
      .insert({
        ...
        session: 1,
        roll_call: 102,
        vote_date: '2026-01-21',
        question: 'On Cloture',
        result: 'Failed',
        bill_id: billA,
        source_url: 'https://congress.gov/vote/102',
      })
      .select()
      .single()
    ...
    const { data: v3 } = await svc
      ...
      .insert({
        ...
        session: 2,
        roll_call: 55,
        vote_date: '2024-09-10',
        question: 'On Passage',
        result: 'Passed',
        bill_id: billA,
        source_url: 'https://congress.gov/vote/118-55',
      })
      .select()
      .single()
