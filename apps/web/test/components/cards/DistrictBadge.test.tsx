...
describe('DistrictBadge', () => {
  it('house variant renders district ordinal', () => {
    render(<DistrictBadge chamber="federal_house" stateName="California" districtNumber={11} />)
  ...
  it('senate variant renders state name only (no district number)', () => {
    render(<DistrictBadge chamber="federal_senate" stateName="California" districtNumber={null} />)
  ...
  it('includes SVG map pin', () => {
    const { container } = render(<DistrictBadge chamber="federal_senate" stateName="California" districtNumber={null} />)
    expect(container.querySelector('svg')).toBeTruthy()
  })
  it('renders 1st/2nd/3rd correctly', () => {
    render(<DistrictBadge chamber="federal_house" stateName="Texas" districtNumber={1} />)
    expect(screen.getByText("Texas's 1st District")).toBeTruthy()
    render(<DistrictBadge chamber="federal_house" stateName="Texas" districtNumber={2} />)
    expect(screen.getByText("Texas's 2nd District")).toBeTruthy()
    render(<DistrictBadge chamber="federal_house" stateName="Texas" districtNumber={3} />)
    expect(screen.getByText("Texas's 3rd District")).toBeTruthy()
    render(<DistrictBadge chamber="federal_house" stateName="Texas" districtNumber={21} />)
