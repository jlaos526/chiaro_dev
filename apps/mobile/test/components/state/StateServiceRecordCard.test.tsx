...
// Avoid loading the real Supabase env module (apps/mobile/lib/env.ts)
// which throws when EXPO_PUBLIC_SUPABASE_URL / _ANON_KEY are absent in the
// jest process. We never hit the network because the data hooks are
// mocked below.
...
describe('mobile StateServiceRecordCard', () => {
  ...
  it('returns null for federal official (chamber-gated)', () => {
    ...
    const { queryByTestId } = render(<StateServiceRecordCard official={fed} />, { wrapper: wrap })
    expect(queryByTestId('state-service-record-card')).toBeNull()
  })
  ...
  it('embeds StateBillsEvidence + StateVotesEvidence panels', () => {
    const { queryByTestId } = render(<StateServiceRecordCard official={mkOfficial()} />, { wrapper: wrap })
    expect(queryByTestId('state-bills-evidence')).not.toBeNull()
    expect(queryByTestId('state-votes-evidence')).not.toBeNull()
  })

  it('renders chamber + party in header row', () => {
    const { getByText } = render(<StateServiceRecordCard official={mkOfficial()} />, { wrapper: wrap })
